{{-- resources/views/gold-purchases/create.blade.php --}}
@extends('layouts.app')

@section('title', 'New Gold Purchase')
@section('page-title', 'New Gold Purchase')
@section('page-description', 'Record a new gold purchase transaction')
...
<div class="max-w-4xl mx-auto space-y-6">
    <form method="POST" action="{{ route('gold-purchases.store') }}" class="space-y-6" x-data="goldPurchaseForm()">
        @csrf
            ...
            <div class="card-body space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" class="form-select @error('type') border-red-500 @enderror" required>
                            ...
                            <option value="pure_gold" {{ old('type') == 'pure_gold' ? 'selected' : '' }}>Pure Gold</option>
                            <option value="used_gold" {{ old('type') == 'used_gold' ? 'selected' : '' }}>Used Gold</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="voucher_no" id="voucher_no" value="{{ old('voucher_no') }}" 
                               placeholder="Auto-generated if empty"
                               class="form-input @error('voucher_no') border-red-500 @enderror">
                        @error('voucher_no')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" 
                              ...
                              placeholder="Additional notes about the purchase...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="card-body space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="karrat_24" id="karrat_24" value="{{ old('karrat_24') }}" 
        ...
        <div class="card">
            ...
            <div class="card-body space-y-4">
                <div>
                    ...
                    <input type="number" name="cash_payment" id="cash_payment" value="{{ old('cash_payment') }}" 
    ...
    return {
        ...
        calculateAmounts() {
            ...
            if (this.weight && this.karat) {
                // Auto-calculate pure weight based on karat
                ...
                if (karatPercentages[this.karat]) {
                    ...
                    this.karrat24 = this.pureWeight; // 24K equivalent is same as pure weight
                }
            }
        }
    }
